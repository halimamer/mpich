...
static inline int MPI_Isend(const void *buf, int count, int dest, int tag)
{
...
.N MPI_ERR_EXHAUSTED
@*/
static inline int MPI_Irecv(void *buf, int count, int source, int tag)
{
    int mpi_errno = MPI_SUCCESS;
...
int PMPI_Isend(const void *buf, int count, int dest, int tag) MPICH_ATTR_POINTER_WITH_TYPE_TAG(1,3) MPICH_API_PUBLIC;
int PMPI_Irecv(void *buf, int count, int source, int tag) MPICH_ATTR_POINTER_WITH_TYPE_TAG(1,3) MPICH_API_PUBLIC;
#endif
