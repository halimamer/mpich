...
#if defined(ENABLE_IZEM)
#define DEC_LOCK_CTXT zm_lock_ctxt_t ctxt
#define GET_CTXT &ctxt
#else
#define DEC_LOCK_CTXT
#define GET_CTXT
#endif
    ...
    do {                                                                \
        if (MPIR_ThreadInfo.isThreaded) {                               \
            ...
            if (per_thread->lock_depth == 0) {                          \
                int err_ = 0;                                           \
                MPIDU_Thread_mutex_lock(&mutex, &err_, GET_CTXT);       \
                MPIR_Assert(err_ == 0);                                 \
            }                                                           \
...
#define MPIDUI_THREAD_CS_ENTER_L_GLOBAL(mutex)                          \
    ...
    do {                                                                \
        if (MPIR_ThreadInfo.isThreaded) {                               \
            ...
            if (per_thread->lock_depth == 0) {                          \
                int err_ = 0;                                           \
                MPIDU_Thread_mutex_lock_l(&mutex, &err_, GET_CTXT);     \
                MPIR_Assert(err_ == 0);                                 \
            }                                                           \
            per_thread->lock_depth++;                                   \
        }                                                               \
...
#define MPIDU_Thread_yield(mutex_ptr_, err_ptr_)                        \
    do {                                                                \
        ...
        MPIR_Assert(*err_ptr_ == 0);                                    \
        MPL_thread_yield();                                             \
        MPIDU_Thread_mutex_lock_l(mutex_ptr_, err_ptr_, NULL);          \
        MPIR_Assert(*err_ptr_ == 0);                                    \
    } while (0)
