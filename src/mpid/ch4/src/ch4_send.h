...
#undef FUNCNAME
#define FUNCNAME MPID_Send_min
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPID_Send_min(const void *buf,
                                        int count,
                                        int rank,
                                        int tag,
                                        int context_offset)
...
#undef FUNCNAME
#define FUNCNAME MPID_Isend_min
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPID_Isend_min(const void *buf,
                                         int count,
                                         int rank,
                                         int tag,
                                         int context_offset)
{
    ...
    if ((r = MPIDI_CH4_rank_is_local(rank, NULL)))
        ...
        mpi_errno =
            MPIDI_NM_mpi_isend_min(buf, count, rank, tag, context_offset);
#endif
    if (mpi_errno != MPI_SUCCESS) {
        MPIR_ERR_POP(mpi_errno);
    }
