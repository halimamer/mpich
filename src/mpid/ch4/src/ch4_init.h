                                        ...
                                        int requested, int *provided, int *has_args, int *has_env)
{
    ...
    MPIDI_CH4_Global.ep_pools = (zm_pool_t*) MPL_malloc(MPIDI_CH4_Global.n_netmod_eps * sizeof(zm_pool_t));
    ...
    for (i = 0; i < MPIDI_CH4_Global.n_netmod_eps; i++) {
        ...
        zm_pool_create(sizeof(MPIDI_workq_elemt_t), &MPIDI_CH4_Global.ep_pools[i]);
...
MPL_STATIC_INLINE_PREFIX int MPID_Finalize(void)
{
    ...
    MPL_free(MPIDI_CH4_Global.ep_queues);
