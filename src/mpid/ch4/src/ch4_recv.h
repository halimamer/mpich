...
#undef FUNCNAME
#define FUNCNAME MPID_Recv_nocomm
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPID_Recv_nocomm(void *buf,
                                        int count,
                                        MPI_Datatype datatype,
                                        int rank,
                                        int tag,
...
#ifndef MPIDI_CH4_EXCLUSIVE_SHM
    mpi_errno =
        MPIDI_NM_mpi_recv_nocomm(buf, count, datatype, rank, tag, context_offset, status, request);
#else
    if (unlikely(rank == MPI_ANY_SOURCE)) {
        mpi_errno =
            MPIDI_SHM_mpi_irecv(buf, count, datatype, rank, tag, NULL /* we assume no shm */, context_offset, request);

        if (mpi_errno != MPI_SUCCESS) {
            MPIR_ERR_POP(mpi_errno);
        }

        mpi_errno = MPIDI_NM_mpi_irecv_nocomm(buf, count, datatype, rank, tag, context_offset,
                                       &(MPIDI_CH4I_REQUEST_ANYSOURCE_PARTNER(*request)));
        ...
        if ((r = MPIDI_CH4_rank_is_local(rank, NULL /* no shm */)))
            mpi_errno =
                MPIDI_SHM_mpi_recv(buf, count, datatype, rank, tag, NULL /* no shm */, context_offset, status,
                                   request);
        else
            mpi_errno =
                MPIDI_NM_mpi_recv_nocomm(buf, count, datatype, rank, tag, context_offset, status,
                                  request);
        if (mpi_errno == MPI_SUCCESS && *request) {
            MPIDI_CH4I_REQUEST(*request, is_local) = r;
            MPIDI_CH4I_REQUEST_ANYSOURCE_PARTNER(*request) = NULL;
...
#undef FUNCNAME
#define FUNCNAME MPID_Irecv_nocomm
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPID_Irecv_nocomm(void *buf,
                                         int count,
                                         MPI_Datatype datatype,
                                         int rank,
                                         int tag,
...
#ifndef MPIDI_CH4_EXCLUSIVE_SHM
    mpi_errno = MPIDI_NM_mpi_irecv_nocomm(buf, count, datatype, rank, tag, context_offset, request);
#else
    if (unlikely(rank == MPI_ANY_SOURCE)) {
        mpi_errno =
            MPIDI_SHM_mpi_irecv(buf, count, datatype, rank, tag, NULL /* we assume no shm */, context_offset, request);
        ...
        int r;
        if ((r = MPIDI_CH4_rank_is_local(rank, NULL /* we assume no shm */)))
            mpi_errno =
                MPIDI_SHM_mpi_irecv(buf, count, datatype, rank, tag, NULL /* we assume no shm */, context_offset, request);
        else
            mpi_errno =
                MPIDI_NM_mpi_irecv_nocomm(buf, count, datatype, rank, tag,  context_offset, request);
        if (mpi_errno == MPI_SUCCESS && *request) {
            MPIDI_CH4I_REQUEST(*request, is_local) = r;
            MPIDI_CH4I_REQUEST_ANYSOURCE_PARTNER(*request) = NULL;
        }
