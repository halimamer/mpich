                                                ...
                                                MPIR_Request ** request, int mode, uint64_t flags)
{
    ...
    *request = rreq;
    ...
    if (!flags) /* Branch should compile out */
        MPIDI_OFI_CALL_RETRY(fi_trecv(MPIDI_Global.ep,
                                      recv_buf,
                                      data_sz,
                                      NULL,
                                      (MPI_ANY_SOURCE ==
        ...
        MPIDI_OFI_CALL_RETRY(fi_trecvmsg(MPIDI_Global.ep, &msg, flags), trecv,
                             MPIDI_OFI_CALL_LOCK);
    }
...
#undef FUNCNAME
#define FUNCNAME MPIDI_OFI_do_irecv_byte
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPIDI_OFI_do_irecv_byte(void *buf,
                                                     int count,
                                                     int rank,
                                                     int tag,
                                                     MPIR_Comm * comm,
                                                     ...
                                                     MPIR_Request ** request, int mode, uint64_t flags)
{
    ...
    *request = rreq;
    ...
    if (unlikely(data_sz > MPIDI_Global.max_send)) {
        ...
        MPIDI_OFI_REQUEST(rreq, event_id) = MPIDI_OFI_EVENT_RECV_BYTE;
...
#undef FUNCNAME
#define FUNCNAME MPIDI_NM_mpi_recv_byte
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPIDI_NM_mpi_recv_byte(void *buf,
                                               int count,
                                               int rank,
                                               int tag,
                                               MPIR_Comm * comm,
                                               ...
                                               MPI_Status * status, MPIR_Request ** request)
{
    ...
    mpi_errno = MPIDI_OFI_do_irecv_byte(buf, count, rank, tag, comm,
                                   context_offset, request, MPIDI_OFI_ON_HEAP, 0ULL);
...
#undef FUNCNAME
#define FUNCNAME MPIDI_NM_mpi_irecv_byte
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPIDI_NM_mpi_irecv_byte(void *buf,
                                                int count,
                                                int rank,
                                                int tag,
                                                MPIR_Comm * comm, int context_offset,
...
{
    int mpi_errno = MPI_SUCCESS;
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_NM_MPI_IRECV_BYTE);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_NM_MPI_IRECV_BYTE);

    mpi_errno = MPIDI_OFI_do_irecv_byte(buf, count, rank, tag, comm,
                                   context_offset, request, MPIDI_OFI_ON_HEAP, 0ULL);

    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_NM_MPI_IRECV_BYTE);
    return mpi_errno;
}
...
MPL_STATIC_INLINE_PREFIX int MPIDI_NM_mpi_cancel_recv(MPIR_Request * rreq)
{
    ...
    ret = fi_cancel((fid_t) MPIDI_Global.ep, &(MPIDI_OFI_REQUEST(rreq, context)));
