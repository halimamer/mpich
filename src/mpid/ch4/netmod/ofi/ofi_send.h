...
#undef FUNCNAME
#define FUNCNAME MPIDI_OFI_send_normal_min
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPIDI_OFI_send_normal_min(const void *buf,int count,
                                                   int rank,int tag,MPIR_Comm *comm,
                                                   int context_offset,MPIR_Request **request,
                                                   size_t data_sz, uint64_t type)
{
    ...
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_OFI_SEND_NORMAL_MIN);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_OFI_SEND_NORMAL_MIN);

    MPIDI_OFI_REQUEST_CREATE(sreq, MPIR_REQUEST_KIND__SEND);
    *request = sreq;
    match_bits =
        MPIDI_OFI_init_sendtag(comm->context_id + context_offset, comm->rank, tag, type);
    MPIDI_OFI_REQUEST(sreq, event_id) = MPIDI_OFI_EVENT_SEND_MIN;
...
#undef FUNCNAME
#define FUNCNAME MPIDI_OFI_send_min
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPIDI_OFI_send_min(const void *buf,int count,
    int rank, int tag,
    int context_offset, int noreq, uint64_t syncflag)
{
    int mpi_errno;
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_OFI_SEND);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_OFI_SEND);
    ...
    if (likely(!syncflag && (count <= MPIDI_Global.max_buffered_send)))
        ...
        MPID_Abort(NULL, MPI_ERR_OTHER, 1, "ERROR: Expected a small contiguous message ");
...
#undef FUNCNAME
#define FUNCNAME MPIDI_NM_mpi_send_min
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
...
#undef FUNCNAME
#define FUNCNAME MPIDI_NM_mpi_isend_min
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
