...
/* -- Begin Profiling Symbol Block for routine MPI_Recv_byte */
#if defined(HAVE_PRAGMA_WEAK)
#pragma weak MPI_Recv_byte = PMPI_Recv_byte
#elif defined(HAVE_PRAGMA_HP_SEC_DEF)
#pragma _HP_SECONDARY_DEF PMPI_Recv_byte  MPI_Recv_byte
#elif defined(HAVE_PRAGMA_CRI_DUP)
#pragma _CRI duplicate MPI_Recv_byte as PMPI_Recv_byte
#elif defined(HAVE_WEAK_ATTRIBUTE)
int MPI_Recv_byte(void *buf, int count, int source, int tag,
             MPI_Comm comm, MPI_Status *status) __attribute__((weak,alias("PMPI_Recv_byte")));
#endif
...
#undef MPI_Recv_byte
#define MPI_Recv_byte PMPI_Recv_byte

#endif

#undef FUNCNAME
#define FUNCNAME MPI_Recv_byte
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)

/*@
    MPI_Recv_byte - Blocking receive for a message
	     ...
	     MPI_Comm comm, MPI_Status *status)
{
    ...
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPI_RECV_BYTE);

    MPIR_ERRTEST_INITIALIZED_ORDIE();
    
    MPID_THREAD_CS_ENTER(GLOBAL, MPIR_THREAD_GLOBAL_ALLFUNC_MUTEX);
    MPIR_FUNC_TERSE_PT2PT_ENTER_BACK(MPID_STATE_MPI_RECV_BYTE);
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    mpi_errno = MPID_Recv_byte(buf, count, source, tag, comm_ptr, 
